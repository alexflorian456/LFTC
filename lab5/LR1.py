            ...
            for element in self.elemente:
                ...
                if membru_dr[pozitie_punct].isupper() and membru_dr[pozitie_punct] not in self.neterminale_verificate:
                    ...
                    for regula in reguli_cu_neterminal:
                        ...
                        #         pass
                        try:
                            self.elemente.append(ElementStare(regula.membru_st, regula.membru_dr, 0, element.predictie + tabel_first1[membru_dr[pozitie_punct+1]]))
                        except IndexError:
                            ...
                            if membru_dr[pozitie_punct+1] not in element.predictie:
                                self.elemente.append(ElementStare(regula.membru_st, regula.membru_dr, 0, element.predictie + [membru_dr[pozitie_punct+1]]))
                    done = False
...
class ColectieCanonica:
    def __init__(self, gramatica, tabel_first1, ter_net): # Goto se face aici
        ...
        self.count_stari = 0
        self.stari = [Stare(self.count_stari, [ElementStare(regula_noua.membru_st, regula_noua.membru_dr, 0, [])], gramatica, tabel_first1)]
        done = False
        self.count_stari += 1
        self.graf = []
        while not done:
                ...
                for t_n in ter_net:
                    ...
                    for element in stare.elemente:
                        ...
                        if element.membru_dr[element.pozitie_punct] == t_n:
                            elemente_initiale_stare_noua.append(ElementStare(element.membru_st, element.membru_dr, element.pozitie_punct+1, element.predictie))
                    if len(elemente_initiale_stare_noua) > 0:
                        ...
                        if stare_noua in self.stari and (stare.index, t_n, self.stari[self.stari.index(stare_noua)].index) not in self.graf:
        ...
        # if len(stare.elemente) == 1:
            if len(stare.elemente[0].predictie) == 0:
                if stare.elemente[0].membru_st == "Z":
                    ...
                    self.tabel[stare.index]["$"] = "r"+str(gramatica.reguli.index(Regula(stare.elemente[0].membru_st, stare.elemente[0].membru_dr)))
            else:
                for pred in stare.elemente[0].predictie:
                    self.tabel[stare.index][pred] = "r"+str(gramatica.reguli.index(Regula(stare.elemente[0].membru_st, stare.elemente[0].membru_dr)))
                self.tabel[stare.index]["c"] = "r"+str(gramatica.reguli.index(Regula(stare.elemente[0].membru_st, stare.elemente[0].membru_dr)))
                ...
                if stare.elemente[0].membru_st in "BCDEFGHI":
                    for letter in "qwertyuiopasdfghjklzxcvbnm":
                        try:
                            ...
                            self.tabel[stare.index][letter] = "r"+str(gramatica.reguli.index(Regula(stare.elemente[0].membru_st, stare.elemente[0].membru_dr)))
                if stare.elemente[0].membru_st in "KLMO":
                    for operator in "*-+!=>":
                        try:
                            self.tabel[stare.index][operator]
                        except KeyError:
                            self.tabel[stare.index][operator] = "r"+str(gramatica.reguli.index(Regula(stare.elemente[0].membru_st, stare.elemente[0].membru_dr)))
                if stare.elemente[0].membru_st in "OQ":
                    for num in "1234567890":
                        try:
                            self.tabel[stare.index][num]
                        except KeyError:
                            self.tabel[stare.index][num] = "r"+str(gramatica.reguli.index(Regula(stare.elemente[0].membru_st, stare.elemente[0].membru_dr)))
...
fip = [9 ,24 ,17 ,19 ,18 ,21 ,13 ,23 ,14 ,20 ,15 ,11 ,10 ,26 ,7 ,(1 , "a"),14 ,25 ,6 ,(1 , "a"),14 ,(1 , "a"),16 ,(1 , "a"),12 ,(0 , "1"),14 ,5 ,8 ,(1 , "a"),8 ,22 ,14 ,27]
fip_error = [9, 24 ,17 ,19 ,18 ,21 ,13 ,23 ,14 ,20 ,15 ,11 ,10 ,7 ,(1 , "a"),14 ,25, 6 ,(1 , "a"),14 ,(1 , "a"),16 ,(1 , "a"),12 ,(0 , "1"),14 ,5 ,8 ,(1 , "a"),8 ,22 ,14 ,27]
